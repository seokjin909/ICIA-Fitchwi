...
const TogetherJoin = ({
  ...
  const getMemberInfo = (id) => {
    axios
      ...
      .then((res) => setInsertFrom({ ...insertForm, memberEmail: res.data }))
    ...
    IMP.request_pay(
      ...
      (rsp) => {
        ...
        if (rsp.success) {
          ...
          axios
            .post("/insertTogetherPayJoinInfo", insertPayForm)
            .then((res) => {
    ...
    IMP.request_pay(
      ...
      (rsp) => {
        ...
        if (rsp.success) {
          ...
          axios
            .post("/insertTogetherPay", insertPayForm)
            .then((res) => {
              setOpen(false);
              swAlert(res.data, "success", () => {
                window.location.reload();
  ...
  const insertTogetherFreeInfo = () => {
    axios.post("/insertTogetherFreeInfo", insertForm.togetherCode).then((res) => {
      setOpen(false);
      swAlert(res.data, "success", () => {
        window.location.reload();
      });
  ...
  const insertTogetherFreeJoinInfo = () => {
    ...
    axios
      .post("/insertTogetherFreeJoinInfo", insertForm)
      .then((res) => {
        setOpen(false);
        swAlert(res.data, "success", () => {
          window.location.reload();
  ...
  const deleteTogetherPayJoinInfo = (e) => {
    ...
    setOpen(false);
    axios
      .delete("/deleteTogetherPayJoinInfo", {
        params: { memberEmail: sessionStorage.getItem("id"), togetherCode: togetherInfo.togetherCode },
      })
      .then((res) => {
        swAlert(res.data, "success");
        window.location.reload();
  ...
  const deleteTogetherFreeJoinInfo = (e) => {
    ...
    setOpen(false);
    axios
      .delete("/deleteTogetherFreeJoinInfo", {
        params: { memberEmail: sessionStorage.getItem("id"), togetherCode: togetherInfo.togetherCode },
      })
      .then((res) => {
        swAlert(res.data, "success", () => {
  ...
  return (
    <>
      {togetherJoinState === "대기" ? (
        <Button onClick={handleOpen} sx={{ width: "100%", mb: 3 }} variant={"contained"}>
          신청취소
        </Button>
      ) : togetherJoinState === "거절" ? (
        <Button variant={"contained"} sx={{ width: "100%", mb: 3 }} disabled>
          신청이 거절되었습니다.
        </Button>
      ) : togetherJoinState === "가입중" ? (
        <Button onClick={handleOpen} sx={{ width: "100%", mb: 3 }} variant={"contained"}>
          참여취소하기
        </Button>
      ) : togetherPayState === "결제완료" ? (
        <Button variant={"contained"} sx={{ width: "100%", mb: 3 }} disabled>
          결제가 완료되었습니다.
        </Button>
      ) : togetherPayState === "삭제신청중" ? (
        <Button variant={"contained"} sx={{ width: "100%", mb: 3 }} disabled>
          삭제 대기중입니다.
        </Button>
      ) : (
        <Button onClick={handleOpen} variant={"contained"} sx={{ width: "100%", mb: 3 }}>
          {children}
        </Button>
      )}
      <Modal
      ...
      >
        {togetherJoinState === "대기" ? (
          <Box sx={style} component="form">
            ...
            {togetherInfo.togetherPrice === 0 && togetherInfo.togetherOpenedCode.facilitiesCode.facilitiesPrice === 0 ? (
              <Button variant="contained" onClick={deleteTogetherFreeJoinInfo} sx={{ mr: 3 }}>
                신청취소
              </Button>
            ) : (
              <Button variant="contained" onClick={deleteTogetherPayJoinInfo} sx={{ mr: 3 }}>
                신청취소(환불)
              </Button>
            )}
          ...
          <Box sx={style}>
            ...
            {togetherInfo.togetherPrice === 0 && togetherInfo.togetherOpenedCode.facilitiesCode.facilitiesPrice === 0 ? (
              <Button variant="contained" type="submit" onClick={deleteTogetherFreeJoinInfo} sx={{ mr: 3 }}>
                취소하기
              </Button>
            ) : (
              <Button variant="contained" type="submit" onClick={deleteTogetherPayJoinInfo} sx={{ mr: 3 }}>
                환불하기
              </Button>
            )}
            <Button variant="contained" onClick={handleClose}>
          ...
          <Box sx={style}>
            ...
            <Button
              ...
              sx={{ mr: 3 }}
              disabled={!(togetherJoinMember.length + 1 === togetherInfo.togetherMax)}
            >
              결제하기
            </Button>
            <Button variant="contained" onClick={handleClose}>
          ...
          <Box sx={style} component="form">
            ...
            <Typography>
              1인당 부담금 :{" "}
              {togetherInfo.togetherPrice + togetherInfo.togetherOpenedCode.facilitiesCode.facilitiesPrice === 0
                ? "무료"
                : togetherInfo.togetherPrice + togetherInfo.togetherOpenedCode.facilitiesCode.facilitiesPrice}
            </Typography>
            {!togetherInfo.togetherInquiry ? (
              <Button type="submit" variant="contained" onClick={togetherJoinSend} sx={{ mt: 2 }}>
                참여하기
              </Button>
            ) : (
              <Button type="submit" variant="contained" onClick={togetherJoinSend} disabled={disabled} sx={{ mt: 2 }}>
                참여하기
              </Button>
            )}
          </Box>
